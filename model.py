...
# TODO:
# 1) Did I use the right YAML file from the repo?
# 2) How do I implement the test function?
...
class LPNModelWrapper:

    def __init__(self):

        # Initialize hydra with the directory containing your config
        with initialize(version_base=None, config_path="."):
            # Compose the config
            self.cfg = compose(config_name="lpn_train")

        if self.cfg.training.get("mixed_precision", False):
            encoder = EncoderTransformer(instantiate_config_for_mpt(self.cfg.encoder_transformer))
            decoder = DecoderTransformer(instantiate_config_for_mpt(self.cfg.decoder_transformer))
        else:
            encoder = EncoderTransformer(hydra.utils.instantiate(self.cfg.encoder_transformer))
            decoder = DecoderTransformer(hydra.utils.instantiate(self.cfg.decoder_transformer))
